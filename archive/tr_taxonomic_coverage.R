...
# setTaxonomicCoverage: main function
...
# identify those taxa for which we can resolve a tsn
...
# convert tibble of taxa - here pulling taxa names from a factors table since
# they were already created.
tr_taxa <- read_csv('plant_attributes_factors.csv') %>% 
  pull(definition)

# run list of taxa through function to identify those taxa for which we can
# resolve a tsn
all_taxa <- identify_resolvable_taxa(tr_taxa)
...
trTaxa <- set_taxonomicCoverage(resolved_taxa, expand = T, db = 'itis')

# set taxonomicCoverage element
coverage@taxonomicCoverage <- c(trTaxa)
